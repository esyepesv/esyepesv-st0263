...
class FileServicer(file_service_pb2_grpc.FileServiceServicer):
    def ListFiles(self, request, context):
        ...
        response = file_service_pb2.ListFilesResponse(filenames=files)
        return response

    def FileExists(self, request, context):
        filename = request.filename
        exists = os.path.isfile(f'../files/{filename}')
        response = file_service_pb2.FileExistsResponse(exists=exists)
        return response
...
def serve():
    ...
    server.start()
    print('Server running at http://localhost:50051')
